...
"""
import time
from resume_enhancer_tab import render_resume_enhancer
...
class ResumeApp:
    ...
    "🚀 RESUME ENHANCER": self.render_resume_enhancer_page,
    ...
    def render_portfolio(self):
        """Render portfolio generation page with proper imports"""
        import streamlit as st
        from utils.portfolio_generator import generate_portfolio, build_template
        ...
        template = st.selectbox(
            "Choose Template",
            ["Modern", "Minimal", "Creative", "Developer", "Professional"]
        )
        ...
        if st.session_state.portfolio_generated:
            ...
            from utils.portfolio_generator import (
                extract_name,
                extract_skills,
                extract_projects,
                extract_experience,
            ...
            if "portfolio_data" not in st.session_state:
                # Extract links
                github = extract_link(resume_text, "github")
                linkedin = extract_link(resume_text, "linkedin")
                about = extract_about(resume_text)
            ...
            if "portfolio_data" in st.session_state:
                ...
                if isinstance(skills_input, list):
                    ...
                    skills_text = str(skills_input)
                
                skills_result = st.text_area(
                    "Skills (comma separated)",
                    skills_text
                )
                data["skills"] = [s.strip() for s in skills_result.split(",") if s.strip()]

                # Handle projects
                projects_input = data.get("projects", [])
                if projects_input and isinstance(projects_input[0], dict):
                    # Structured projects
                    projects_text = "\n".join([p.get("title", "") for p in projects_input])
                else:
                    projects_text = "\n".join([str(p) for p in projects_input])
                
                projects_result = st.text_area(
                ...
                data["projects"] = [
                    {"title": line.strip(), "description": line.strip(), "tech": []}
                    for line in projects_result.split("\n") if line.strip()
                ]

                # Handle experience
                exp_input = data.get("experience", [])
                if exp_input and isinstance(exp_input[0], dict):
                    exp_text = "\n".join([e.get("title", "") for e in exp_input])
                else:
                    exp_text = "\n".join([str(e) for e in exp_input])
                
                exp_result = st.text_area(
                ...
                # Convert to structured format
                data["experience"] = [
                    {"title": line.strip(), "company": "", "description": [line.strip()]}
                    for line in exp_result.split("\n") if line.strip()
                ]

                # Handle education
                edu_input = data.get("education", [])
                if edu_input and isinstance(edu_input[0], dict):
                    edu_text = "\n".join([e.get("degree", "") for e in edu_input])
                else:
                    edu_text = "\n".join([str(e) for e in edu_input])
                
                edu_result = st.text_area(
                ...
                # Convert to structured format
                data["education"] = [
                    {"degree": line.strip(), "institution": ""}
                    for line in edu_result.split("\n") if line.strip()
                ]

                data["profile_image"] = st.text_input(
                    "Profile Image URL",
                ...
                try:
                    ...
                    st.success("✅ Your portfolio is ready to be deployed!")
                    ...
                    with col2:
                        ...
                        st.download_button(
                            ...
                            zip_buffer,
                            file_name="portfolio_site.zip",
                            mime="application/zip",
                            use_container_width=True
                        )
                    
                    st.markdown("""
                    ### 🌐 How to Publish Your Portfolio (GitHub Pages)

                    1. Go to 👉 https://github.com/new  
                    2. Create a new repository (name: `portfolio`)  
                    3. Extract the ZIP and upload the `index.html` file  
    ...
    def render_resume_enhancer_page(self):
        render_resume_enhancer()
                                ...
                                if analysis_result and "error" not in analysis_result:
                                    ...
                                    <div style="background-color: #262730; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
                                        ...
                                        <div style="display: flex; flex-wrap: wrap; gap: 20px;">
                                            ...
                                            <div style="flex: 1; min-width: 200px;">
                                                ...
                                                <p style="color: #ffffff;"><strong>Overall Score:</strong> {ats_score}/100 - {"Excellent" if ats_score >= 80 else "Good" if ats_score >= 60 else "Needs Improvement"}</p>
                                    ...
                                    for section in sections:
                                        ...
                                        with st.container():
                                            ...
                                            st.markdown(f"""
                                            <h3 style="color:#4da6ff; margin-bottom:10px;">
                                            {title}
                                            </h3>
                                            """, unsafe_allow_html=True)

                                            # 🔥 SKILLS SECTION SPECIAL
                                            if "Skills Analysis" in title:
                                                for line in content:
                                                    line = line.strip()

                                                    if "Current Skills" in line:
                                                        st.markdown("""
                                                        <h4 style="color:#00e676; text-decoration:underline;">
                                                        Current Skills
                                                        </h4>
                                                        """, unsafe_allow_html=True)
                                                        ...
                                                        st.markdown("""
                                                        <h4 style="color:#ff5252; text-decoration:underline;">
                                                        Missing Skills
                                                        </h4>
                                                        """, unsafe_allow_html=True)
                                            ...
                                            # 🔥 NORMAL SECTIONS
                                                ...
                                                for line in content:
                                                    ...
                                                    if line.startswith("-"):
                                                        ...
                                                        st.markdown(f"<p style='color:#ccc'>{line}</p>", unsafe_allow_html=True)
