...
- NO inline bullets like: "text - text - text"
- Use clean format:
  - Point 1
  - Point 2
- Max 4–5 bullets per section
...
"""
    ...
    def generate_pdf_report(self, analysis_result, candidate_name, job_role):
        ...
        try:
            ...
            # Analysis Content
            analysis_text = analysis_result.get("full_response") or analysis_result.get("analysis") or ""
            ...
            content.append(Paragraph("ATS Evaluation", heading_style))

            score_table_data = [
                ["ATS Score"],
                [GaugeChart(width=300, height=200, score=ats_score, max_score=100, label="ATS Score")]
            ]
            ...
            for line in overall_assessment.split("\n"):
                line = line.strip()
                if not line:
                    continue
                if line.startswith("-"):
                    content.append(Paragraph("• " + clean_markdown(line[1:].strip()), list_item_style))
                else:
                    content.append(Paragraph(clean_markdown(line), normal_style))
                ...
                if section_title == "Skills Analysis":
                    ...
                    for para in section_content.split("\n"):
                        ...
                        if para.startswith("-") or para.startswith("*") or para.startswith("•"):
                            para = "• " + clean_markdown(para[1:].strip())
                            content.append(Paragraph(para, list_item_style))
                        else:
